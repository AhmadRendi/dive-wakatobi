...
class Booking {
    ...
    public function savePemesanan($data){
        try{
            $query = "INSERT INTO $this->table (id_user, id_paket, id_guide, id_keahlian, namaLengkap, tanggalPemesanan, status, jmlPeserta, harga, noHp) "
                    . "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $this->db->query($query);
